...
| O front-end tem verificação automatizada no servidor | build como portão que **reprova de fato** (conferido quebrando um import) e os **80 testes de ponta a ponta** contra banco efêmero, sem nenhum segredo; lint com **11 erros** visíveis, ainda sem reprovar | ESLint 9 sobre 173 arquivos: 25 erros na primeira execução, 14 tratados; PostgreSQL do runner semeado do zero, chave Fernet gerada na execução | `FE/.github/workflows/ci.yml` |
